...
更新联盟网络分析参数后，重新运行全部 5 个委员会的共同提案联盟网络分析。当前结果统计：

| 委员会 | 节点数 | 边数 | 共同提案数 | 社群数 |
|--------|--------|------|------------|--------|
| MEPC | 25 | 181 | 1164 | 4 |
| MSC | 25 | 52 | 197 | 6 |
| SSE | 25 | 82 | 265 | 4 |
| CCC | 25 | 36 | 86 | 9 |
| ISWG-GHG | 25 | 179 | 497 | 4 |

所有委员会的 alliance 文件夹已更新（网络图、国家统计、配对排名、解读文本）。

---

### 2. 输出文件清理

清理 `output/` 目录中的冗余与过期文件：

- **可视化文件命名规范统一**：删除 5 个委员会 `visualization/` 文件夹中旧命名格式（`{COMMITTEE}_xxx`），保留新命名格式（`xxx_{COMMITTEE}`）。每个委员会删除 6 个旧文件，共 30 个。MEPC 额外删除 `MEPC_topic_heatmap.png` 和 `topic_meeting_heatmap_MEPC.csv`。
- **MEPC 会议级旧分析产物**：删除 MEPC 77-83 各会议文件夹中的 `cooccurrence_graph.png`、`word_freq.json`、`processed_tokens.json`（共 21 个），以及 MEPC 77/82 的 `Analysis_Report_Full.html`、MEPC 77/78 的 `document_topic_distribution.csv`、`topics_summary.txt`、`topics_table.csv` 等。MEPC 77 还清理了 `per_agenda` 子文件夹、`cooccurrence_overall.png`、`data.json.bak` 等。
- **空文件夹**：删除无效的 `CCC/CCC 0` 空文件夹。
- **MEPC 根目录**：删除旧的 `MEPC_Analysis_Report.html` 和 `trends/` 文件夹（2025年12月的旧趋势分析）。

清理后 output 文件数从 516 降至 364。

---

### 3. 毕业论文 output 同步

将清理后的 Research output 同步至毕业论文目录 `毕业设计/毕业论文/output/`：

- 同步范围：alliance、visualization、bertopic、citation、cooccurrence、deep_analysis、dynamic_analysis、stance_analysis、顶层 visualization
- 排除项：原始数据文件（data.json/xlsx/processed/parsed）、BERTopic 模型文件、JSON 统计报告
- 同时清理了毕业论文 output 中的旧命名格式可视化文件

同步后毕业论文 output 文件数从 317 降至 241（移除冗余后的精简版本）。
...
| 2026-03-29 | 更新 | output/{委员会}/alliance/ | 联盟网络参数更新，5个委员会重新运行 |
| 2026-03-29 | 清理 | output/{委员会}/visualization/ | 删除旧命名格式文件（30+2个） |
| 2026-03-29 | 清理 | output/MEPC/MEPC 77-83/ | 删除会议级旧分析产物 |
| 2026-03-29 | 清理 | output/CCC/CCC 0 | 删除无效空文件夹 |
| 2026-03-29 | 清理 | output/MEPC/ | 删除旧报告和 trends 文件夹 |
| 2026-03-29 | 同步 | 毕业论文/output/ | 同步最新结果至毕业论文目录 |
| 2026-03-29 | 更新 | 研究日志.md | 记录联盟网络更新、文件清理、论文同步 |
